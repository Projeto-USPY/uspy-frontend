...
const OfferingReviewReportDialog: React.FC<PropsType> = ({
	review,
	isOpen,
	close,
}) => {
	const [comment, setComment] = useState<string>('')
	const { course, specialization, code, professor } =
		useContext(OfferingContext)
	const notify = useMySnackbar()

	const handleSubmit = useCallback(() => {
		// fazer alguma coisa com api
		api.reportOfferingReview(
			course,
			specialization,
			code,
			professor,
	...
	return (
		<Dialog open={isOpen} onClose={close}>
			...
			<DialogContent>
				<DialogContentText>
					Por favor, descreva por que esse comentário deve ser
					reportado. Caso ele seja ofensivo, preconceituoso,
					calunioso, irrelevante ao oferecimento ou, por qualquer
					outra razão, não deva estar aqui, nós tomaremos providências
					deletando o comentário e possivelmente banindo a conta do
					autor(a).
				</DialogContentText>
				<ReportField
					autoFocus
