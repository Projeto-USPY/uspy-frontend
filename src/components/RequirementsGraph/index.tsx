...
const Box: React.FC<BoxProps> = ({ code, name, isLink, strong, relations }) => {
	...
	const handleClick = () => {
		history.push(
			buildSubjectURI(params.course, params.specialization, code),
		)
	}
...
const RequirementsGraph: React.FC<RequirementsGraphProps> = ({
	...
	const cardHeight =
		Math.max(predecessors?.length, successors?.length, 1) * 50 + 30 // calculate cardHeight from size of arrays
	...
	const content = (
		<Grid
			...
			<Grid
				...
				/>
				<Typography variant="caption">Você está aqui</Typography>
			</Grid>
			<Grid
				container
				item
		...
		<>
			...
			<ArcherContainer strokeColor="black" strokeWidth={1}>
				{isLoading ? (
					...
					<MessagePanel
						height={200}
						message="Sem requisitos ou trancamentos"
					/>
				) : (
					content
				)}
