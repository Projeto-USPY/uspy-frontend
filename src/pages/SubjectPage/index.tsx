...
function getSubjectRequirementsList(sub: Subject) {
	if (!sub.requirements.length || !sub.requirements[0].length)
		throw new Error(
...
export function getMeta(): any {
	return {
		title: 'USPY',
		description:
			'Veja a disciplina, seus requisitos, créditos, oferecimentos, médias e avalie-a!',
		robots: ['index', 'follow'],
	}
}
...
const SubjectPage = ({ user }: SubjectPageProps) => {
	...
	const [subjectReview, setSubjectReview] = useState<SubjectReview | null>(
		null,
	)
		...
		api.getSubjectReview(course, specialization, code)
			...
			.catch(err => {
				if (err.code === 'not_found') {
					...
					// either user is not logged in or user was not enrolled in subject
					setEvaluateSubject(false)
				}
			})
	...
	const handleReviewSubject = (c: 'S' | 'N') => {
		...
		if (subjectReview !== null) {
			...
			newSubject.stats.worth_it -= subjectReview.categories.worth_it
				? 1
				: 0
		}
	...
	if (canSeeChart && gradeStats) {
		if (gradeStats.grades && Object.keys(gradeStats.grades).length > 0) {
			chartContent = (
				<GradeDistributionChart
					grades={copyObj(gradeStats.grades)}
			...
			chartContent = (
				<MessagePanel
					height={200}
					message="Não há dados suficientes para mostrar esse recurso"
				/>
			)
		}
	} else {
		...
		chartContent = (
			<MessagePanel
				...
				actionTitle="Entrar"
				message="Você precisa estar logado para ter acesso a esse recurso"
			/>
		)
	}
						...
						<Grid item xs={12}>
							<Card elevation={3} className="prompt">
								...
								<CardContent>
									...
									Requisitos:{' '}
									{subject.requirements.length
										? getSubjectRequirementsList(subject)
										: 'Nenhum'}
									<br />
								...
								<CardContent>
									...
									{evaluateSubject ? (
										<SubjectEvaluationRadio
											...
											clickCallback={handleReviewSubject}
										/>
									) : (
										<></>
									)}
										...
										<>
											<p>
												...
												{getRecommendationRate(
													subject?.stats?.worth_it,
													subject?.stats?.total,
												)}
												% dos alunos dizem que essa
												disciplina vale a pena!{' '}
											</p>
											<p>
												{' '}
												Total de reviews:{' '}
