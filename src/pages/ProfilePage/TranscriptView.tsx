...
const TranscriptView: React.FC<TranscriptViewPropsType> = ({ semesters }) => {
	...
	const [subjectInReview, setSubjectInReview] = useState<SubjectKey | null>(
		null,
	)
		...
		>
			{[0, 1].map((idx) => (
				<Button
					...
					disabled={
						pending ||
						semesters.indexOf(2 * selectedYear + idx) === -1
					}
	...
	const navigator = isDesktop ? (
		<Grid container justify="space-between">
			<Grid item>{semesterSelector}</Grid>
			<Grid item>
				<div>
					{[KeyboardArrowLeftIcon, KeyboardArrowRightIcon].map(
						(Icon, idx) => (
							<IconButton
								color="secondary"
								key={idx}
